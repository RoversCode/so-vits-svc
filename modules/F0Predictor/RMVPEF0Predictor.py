...
class RMVPEF0Predictor(F0Predictor):
    ...
    def repeat_expand(
        ...
        results = torch.nn.functional.interpolate(
            content, size=int(target_len), mode=mode
        )
    ...
    def post_process(
        self, x, sampling_rate, f0, pad_to
    ):  # self.post_process(x, self.sampling_rate, f0, p_len)
        ...
        f0 = (
            torch.index_select(f0, dim=0, index=nzindex).cpu().numpy()
        )  # 只保留非零F0值
        time_org = (
            self.hop_length / sampling_rate * nzindex.cpu().numpy()
        )  # origin time
