...
def main(args):
    ...
    # 切片并重新命名
    '''
    role_name = os.path.basename(args.audio_dir)
    # 新建文件夹
    output_dir = os.path.join('dataset_raw', role_name)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    else:
        print(f"warning: {output_dir} already exists, please remove it first.")    
    ...
    for file_name in tqdm(files):
        audio, sr = librosa.load(os.path.join(args.audio_dir, file_name), sr=args.sr)
            ...
            if len(chunk) > max_length * sr:
                ...
                for c in second_chunks:
                    if len(c) > max_length * sr:
                        ...
                        for n in range(num):
                            soundfile.write(
                                os.path.join(output_dir, f"{role_name}_{file_idx:06d}.wav"),
                                c[n * time_inter * sr : (n + 1) * time_inter * sr],
                                sr,
                            )
                            file_idx += 1
                        if len(c[num * time_inter * sr :]) > args.audio_min_length * sr:
                            soundfile.write(
                                os.path.join(output_dir, f"{role_name}_{file_idx:06d}.wav"),
                                c[num * time_inter * sr :],
                                sr,
