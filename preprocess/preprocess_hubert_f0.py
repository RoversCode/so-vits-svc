...
def process_one(filename, hmodel, f0p, device, diff=False, mel_extractor=None):
    ...
    if not os.path.exists(f0_path):
        f0_predictor = utils.get_f0_predictor(f0p,
    ...
    if diff:
        ...
        if mel_extractor is not None:
            aug_mel_t = mel_extractor.extract(audio_norm *
                                              (10**log10_vol_shift),
                                              sampling_rate,
                                              keyshift=keyshift)
        ...
        if not os.path.exists(aug_mel_path):
            np.save(aug_mel_path,
                    np.asanyarray((aug_mel, keyshift), dtype=object))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_diff',
                        action='store_true',
                        help='Whether to use the diffusion model')
    parser.add_argument(
        '--f0_predictor',
        type=str,
        default="rmvpe",
        help=
        'Select F0 predictor, can select crepe,pm,dio,harvest,rmvpe,fcpe|default: pm(note: crepe is original F0 using mean filter)'
    )
    parser.add_argument(
        '--num_processes',
        type=int,
        default=1,
        help=
        'You are advised to set the number of processes to the same as the number of CPU cores'
    )
    ...
    if args.use_diff: # diffusion
        ...
        mel_extractor = Vocoder(dconfig.vocoder.type,
                                dconfig.vocoder.ckpt,
                                device=device)
    ...
    parallel_process(filenames, num_processes, f0p, args.use_diff,
                     mel_extractor, device)
