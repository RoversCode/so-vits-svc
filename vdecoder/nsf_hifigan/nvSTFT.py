...
class STFT:
    ...
    def get_mel(self, y, keyshift=0, speed=1, center=False):
        ...
        factor = 2 ** (keyshift / 12)  # 12-semitone key shift
        n_fft_new = int(np.round(n_fft * factor))  # 
        win_size_new = int(np.round(win_size * factor))  # 窗口变大
        ...
        if keyshift_key not in self.hann_window:
            self.hann_window[keyshift_key] = torch.hann_window(win_size_new).to(
                y.device
            )
