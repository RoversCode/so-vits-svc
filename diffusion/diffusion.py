...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "spec_min", torch.FloatTensor([spec_min])[None, None, :out_dims]
        )
        self.register_buffer(
            "spec_max", torch.FloatTensor([spec_max])[None, None, :out_dims]
        )
    ...
    def q_sample(self, x_start, t, noise=None):  # 加噪
    ...
    def p_losses(self, x_start, t, cond, noise=None, loss_type="l2"):
        ...
        x_recon = self.denoise_fn(x_noisy, t, cond)  # 预测t时刻下的噪声分布
    ...
    def forward(
        ...
        if not infer:
            ...
            if gt_spec is None:
                ...
                x = self.q_sample(
                    x_start=norm_spec, t=torch.tensor([t - 1], device=device).long()
                )
            ...
            if method is not None and infer_speedup > 1:
                if method == "dpm-solver" or method == "dpm-solver++":
                    ...
                    noise_schedule = NoiseScheduleVP(
                        schedule="discrete", betas=self.betas[:t]
                    )
                ...
                if use_tqdm:
                    for i in tqdm(
                        reversed(range(0, t)), desc="sample time step", total=t
                    ):
                        x = self.p_sample(
                            x,
                            torch.full((b,), i, device=device, dtype=torch.long),
