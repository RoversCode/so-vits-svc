...
def get_data_loaders(args, whole_audio=False):
    ...
    loader_train = torch.utils.data.DataLoader(
        ...
        persistent_workers=(
            (args.train.num_workers > 0) if args.train.cache_device == "cpu" else False
        ),
...
class AudioDataset(Dataset):
        ...
        for name_ext in tqdm(self.paths, total=len(self.paths)):
            ...
            if n_spk is not None and n_spk > 1:
                ...
                if spk_id < 0 or spk_id >= n_spk:
                    raise ValueError(
                        " [x] Muiti-speaker traing error : spk_id must be a positive integer from 0 to n_spk-1 "
                    )
            else:
                spk_id = 0
    ...
    def get_data(self, name_ext, data_buffer):
        ...
        if units is None:
            ...
            units = units[0]
            units = repeat_expand_2d(
                units, f0.size(0), self.unit_interpolate_mode
            ).transpose(0, 1)
