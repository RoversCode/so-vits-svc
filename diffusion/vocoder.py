...
class Vocoder:
    ...
    def extract(self, audio, sample_rate, keyshift=0):
        ...
        mel = self.vocoder.extract(audio_res,
                                   keyshift=keyshift)  # B, n_frames, bins
        return mel
...
class NsfHifiGAN(torch.nn.Module):
    ...
    def __init__(self, model_path, device=None):
        ...
        self.stft = STFT(self.h.sampling_rate, self.h.num_mels, self.h.n_fft,
                         self.h.win_size, self.h.hop_size, self.h.fmin,
                         self.h.fmax)
    ...
    def forward(self, mel, f0):
        if self.model is None:
            ...
            self.model, self.h = load_model(self.model_path,
                                            device=self.device)
        with torch.no_grad():
            c = mel.transpose(1, 2)
