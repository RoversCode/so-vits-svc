...
def modelAnalysis(
    ...
    try:
        ...
        model = Svc(
            ...
            cluster_model_path=(
                cluster_model_path.name if cluster_model_path is not None else ""
            ),
            nsf_hifigan_enhance=enhance,
            diffusion_model_path=(
                diff_model_path.name if diff_model_path is not None else ""
            ),
            diffusion_config_path=(
                diff_config_path.name if diff_config_path is not None else ""
            ),
...
def vc_fn(
    ...
    try:
        ...
        if (
            getattr(model, "cluster_model", None) is None
            and model.feature_retrieval is False
        ):
            if cluster_ratio != 0:
                return (
                    "You need to upload an cluster model or feature retrieval model before assigning cluster ratio!",
                    None,
                )
        # print(input_audio)
                    ...
                    with gr.Tabs():
                        ...
                        with gr.TabItem("本地") as local_model_tab_local:
                            ...
                            local_model_selection = gr.Dropdown(
                                label="选择模型文件夹", choices=[], interactive=True
                            )
                    with gr.Row():
            ...
            with gr.Row(variant="panel"):
                with gr.Column():
                    ...
                    vc_transform = gr.Number(
                        label="变调（整数，可以正负，半音数量，升高八度就是12）",
                        value=0,
                    )
                    ...
                    k_step = gr.Slider(
                        label="浅扩散步数，只有使用了扩散模型才有效，步数越大越接近扩散模型的结果",
                        value=100,
                        minimum=1,
                        maximum=1000,
                    )
                with gr.Column():
                    ...
                    second_encoding = gr.Checkbox(
                        label="二次编码，浅扩散前会对原始音频进行二次编码，玄学选项，效果时好时差，默认关闭",
                        value=False,
                    )
                    use_spk_mix = gr.Checkbox(
                        label="动态声线融合", value=False, interactive=False
                    )
            with gr.Tabs():
            ...
            with gr.Tabs():
                with gr.TabItem("静态声线融合"):
                    ...
                    mix_model_upload_button = gr.UploadButton(
                        "选择/追加需要混合模型文件", file_count="multiple"
                    )
                    mix_model_output1 = gr.Textbox(
                        label="混合比例调整，单位/%", interactive=True
                    )
                    mix_mode = gr.Radio(
                        choices=["凸组合", "线性组合"],
                        label="融合模式",
                    ...
                    mix_model_output2 = gr.Textbox(label="Output Message")
                    mix_model_path.change(
                        updata_mix_info, [mix_model_path], [mix_model_output1]
                    )
                ...
                with gr.TabItem("模型压缩工具"):
                    ...
                    compress_model_btn.click(
                        model_compression, [model_to_compress], [compress_model_output]
                    )
        ...
        with gr.Row(variant="panel"):
            with gr.Column():
                ...
                debug_button = gr.Checkbox(
                    label="Debug模式，如果向社区反馈BUG需要打开，打开后控制台可以显示具体错误提示",
                    value=debug,
                )
        # refresh local model list
        local_model_refresh_btn.click(
            local_model_refresh_fn, outputs=local_model_selection
        )
